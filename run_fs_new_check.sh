...
do
    ...
    do
        ...
        do
            for st in 1500 
            do
            python -u pl_train.py -k  exp_name=${ds}_shots${shot}_seed${seed}_fs_stage2 few_shot_random_seed=${seed} seed=${seed} dataset=${ds} batch_size=1 grad_accum_factor=2 num_steps=${st} eval_batch_size=4 num_shot=${shot} stage=2 load_weight=${OUTPUT_PATH}/${ds}_shots${shot}_seed${seed}_fs_stage1/finish.pt
